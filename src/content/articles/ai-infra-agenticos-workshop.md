...
类似的思路也出现在安全领域。论文《VibeWAF》探索了用 LLM 来生成 WAF 规则。传统的规则维护依赖安全专家手动编写和更新，速度慢且对专业知识要求高。VibeWAF 采用了一个混合架构：快速的规则引擎处理已知的攻击模式，LLM 分析规则引擎未能匹配的流量并生成新规则，反哺到规则引擎中覆盖未来的类似请求。这个反馈循环收敛到 **88% 的规则命中率**，平均延迟从 6.5 秒降到 **400 毫秒**以内。
